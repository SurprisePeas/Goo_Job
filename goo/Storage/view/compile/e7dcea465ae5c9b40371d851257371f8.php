				...
				<tbody>
					<?php foreach ($nodeData as $v){?>
						...
						<?php foreach ($v['_data'] as $value){?>
							<tr>
								<td>
									<laber><input type="checkbox" name="nodeid[]" class="parent" value="<?php echo $value['id']?>" /><?php echo $value['title']?></laber></td>
								<td>
									<?php foreach ($value['_data'] as $son){?>
										<label>
											<input type="checkbox" <?php if(in_array($son['id'], $old_node_id)): ?> checked <?php endif ?>  class="son" name="nodeid[]" value="<?php echo $son['id']?>" />
											<?php echo $son['title']?>
										</label>
									<?php }?>
								</td>
