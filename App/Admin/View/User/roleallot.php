				...
				<tbody>
					{{foreach from="$nodeData" value="$v"}}
						...
						{{foreach from="$v['_data']" value="$value"}}
							<tr>
								<td>
									<laber><input type="checkbox" name="nodeid[]" class="parent" value="{{$value['id']}}" />{{$value['title']}}</laber></td>
								<td>
									{{foreach from="$value['_data']" value="$son"}}
										<label>
											<input type="checkbox" <?php if(in_array($son['id'], $old_node_id)): ?> checked <?php endif ?>  class="son" name="nodeid[]" value="{{$son['id']}}" />
											{{$son['title']}}
										</label>
									{{endforeach}}
								</td>
